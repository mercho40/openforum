...
import { VerifyEmailForm } from "@/components/new/auth/VerifyEmailForm";
...
export default async function Page() {
  ...
  let error = null;
    ...
    const HanldeVerificationComplete = async () => {
        redirect("/auth/callback");
    }
    ...
    return (
        <main className="flex min-h-[100dvh] w-full flex-col items-center justify-center p-4">
            ...
            <Suspense fallback={<Loader2 className="animate-spin text-muted-foreground" />}>
                <VerifyEmailForm email={session.user.email} onVerificationComplete={HanldeVerificationComplete} />
            </Suspense>
        </main>
    )
}
