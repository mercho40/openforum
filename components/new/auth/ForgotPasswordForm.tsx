...
export function ForgotPasswordForm({ email: initialEmail = "", onForgotComplete }: ForgotPasswordProps) {
        ...
        try {
            ...
            const interval = setInterval(() => {
                ...
                setResendOtpTimer((prev) => {
                    // If timer reaches 0, clear interval and enable resend button
                    if (prev <= 0) {
    ...
    const handleResetPassword = async () => {
        ...
        // Ensure OTP code is sent
        if (!isCodeSent) {
            toast.error("Please send the reset code first")
            setOtpInputs(["", "", "", "", "", ""]) // Reset inputs
            return
        }
        ...
        // Ensure OTP code is valid
        if (otpInputs.some((digit) => digit === "")) {
            toast.error("Please fill all 6 digits of the verification code")
            setOtpInputs(["", "", "", "", "", ""]) // Reset inputs
            return
        }
        
        // Get the complete OTP code from inputs
